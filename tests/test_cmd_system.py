...
def test_up_requires_fin_app(monkeypatch, tmp_path):
    monkeypatch.setattr(system.ProjectEnv, "load", classmethod(lambda cls: _env(tmp_path)))
...
def test_up_unknown_plug(monkeypatch, tmp_path):
    monkeypatch.setattr(system.ProjectEnv, "load", classmethod(lambda cls: _env(tmp_path, FIN_APP="ghost")))
...
def test_up_wrong_plug_type(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(system.ProjectEnv, "load", classmethod(lambda cls: _env(tmp_path, FIN_APP="mysql")))
...
def test_up_happy_path(monkeypatch, tmp_path):
    ...
    class MyApp(FinPlug):
        ...
        def primary_spec(self, env):
            return ContainerSpec(service="web", image="demo:latest", workdir_mount="/app")
    ...
    calls = {}
    monkeypatch.setattr(system.ProjectEnv, "load",
                        classmethod(lambda cls: _env(tmp_path, FIN_APP="myapp", FIN_SITE="app.localhost")))
    monkeypatch.setattr(system, "load_by_name", lambda name: FakeLP())
    monkeypatch.setattr(system, "ensure_proxy", lambda: calls.setdefault("proxy", True))
    monkeypatch.setattr(system, "start_assets_for", lambda env: calls.setdefault("assets", []) or [])
    monkeypatch.setattr(system, "start_primary", lambda spec, env: calls.setdefault("primary", spec))
    monkeypatch.setattr(system, "ensure_project_database", lambda env: calls.setdefault("db", True))
...
def test_up_validates_env_contract(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(system.ProjectEnv, "load",
                        classmethod(lambda cls: _env(tmp_path, FIN_APP="strict")))  # no FIN_SITE
    monkeypatch.setattr(system, "load_by_name", lambda name: FakeLP())
